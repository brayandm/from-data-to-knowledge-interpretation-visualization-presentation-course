...
BCN <- read_sf(dsn = ".", layer = "BCN_CT")

plot(st_geometry(BCN))

num_polygons <- nrow(BCN)
print(paste("Number of polygons:", num_polygons))

crs_BCN <- st_crs(BCN)
print(paste("CRS:", crs_BCN))

bbox_BCN <- st_bbox(BCN)
print(paste("Bounding box:", bbox_BCN))
...
BCN_districts$centroid <- st_centroid(BCN_districts$geometry)

base_palette <- brewer.pal(n = 12, name = "Set3")
color_palette <- colorRampPalette(base_palette)(length(unique(BCN_districts$NAME)))

map <- ggplot() +
  geom_sf(data = BCN, aes(fill = NAME), color = "white", lwd = 0.2) +
  scale_fill_manual(values = color_palette) +
  geom_sf(data = BCN_districts, fill = NA, color = "gray", lwd = 0.5) +
 geom_text_repel(data = BCN_districts, aes(label = NAME, geometry = centroid),
                  stat = "sf_coordinates", size = 4, min.segment.length = 0, color = "black",
  ...
  labs(title = "Districts and Census Tracts of Barcelona")

ggsave("plot2.png", plot = map, width = 10, height = 8)
```