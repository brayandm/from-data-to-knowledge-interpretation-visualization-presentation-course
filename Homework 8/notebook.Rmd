...
bbox <- st_bbox(bars)
expanded_bbox <- bbox + c(-0.05, -0.05, 0.05, 0.05) # Ajusta estos valores según lo necesario

# Crear una cuadrícula hexagonal usando la caja delimitadora ampliada
hex_grid <- st_make_grid(st_as_sfc(expanded_bbox), cellsize = c(0.005, 0.005), what = "polygons", square = FALSE)
hex_grid_sf <- st_sf(hex_grid) %>%
  mutate(grid_id = 1:length(lengths(hex_grid)))

# Intersecar con el municipio de Madrid
madrid_hexa <- st_intersection(madrid_municipality, hex_grid_sf)

# Contar los puntos en cada hexágono
...
```{r, cache=TRUE} 
# Definir los colores para el mapa
...
MAP4 <- ggplot(madrid_hexa, aes(fill = pt_count)) + 
  ...
  scale_fill_distiller(palette = "Spectral", name = "Número de bares", na.value = "#EBEBEB") +
  labs(
    x = "\nLongitud",
    y = "Latitud\n",
    title = "Mapa Hexagonal de Bares en Madrid",
    subtitle = "Datos de OpenStreetMap",
    caption = "Elaboración: Tu Nombre\nDatos: OpenStreetMap"
  ) +
  theme(
...
# Guardar el mapa final como imagen
