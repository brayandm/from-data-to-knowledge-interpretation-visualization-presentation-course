...
```{r, cache=TRUE} 
stadia_api_key <- "a27851bf-c50f-45d2-956d-5dad97b98625"
register_stadiamaps(stadia_api_key)

hex_grid <- st_make_grid(bars, cellsize = c(0.0025, 0.0025), what = "polygons", square = FALSE)
...
plot <- ggplot() +
  ...
  geom_sf(data = data_sf, aes(size = deaths, color = deaths_category), alpha = 0.7) +
  scale_size(range = c(1, 10), name = "Number of Deaths") +
  scale_color_manual(values = c("1-10" = "blue", "11-50" = "green", "51-100" = "yellow", "101-200" = "orange", ">200" = "red"), name = "Deaths Category") +
  theme_minimal() +
  labs(title = "Missing Migrants in the Mediterranean", x = "Longitude", y = "Latitude")
