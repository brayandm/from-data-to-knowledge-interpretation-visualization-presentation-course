...
# Agrupar por universidad y sumar las medallas
...
# Agrupar por país y sumar las medallas
...
# Definir los países de América Latina y América del Norte
latin_america_regions <- c("Central America", "South America", "Caribbean")
north_america_regions <- c("Northern America")

# Asignar los continentes específicos
icpc_data <- icpc_data %>%
  mutate(Continent = case_when(
    Region %in% latin_america_regions ~ "Latin America",
    Region %in% north_america_regions ~ "North America",
    TRUE ~ Continent
  ))

# Agrupar por continente y sumar las medallas
...
```{r}
# Gráfico de barras horizontales para las universidades
...
# Gráfico de barras horizontales para los países
...
```{r}
# Gráfico de barras horizontales para los continentes
...
plot <- ggplot(continent_medals, aes(x = reorder(Continent, Total_Medals), y = Total_Medals)) +
  ...
  coord_flip() +
  scale_y_continuous(breaks = seq(0, 156, by = 12)) +  # Ajusta los límites aquí
  labs(
