...
bcn_districts_df <- st_as_sf(bcn_districts)
bcn_districts_df <- cbind(bcn_districts_df, st_coordinates(st_centroid(bcn_districts_df)))
...
heatmap <- ggmap(bcn_map) +
  ...
  geom_label(data = bcn_districts_df, aes(x = X, y = Y, label = NOM), 
             size = 3, color = "black", fill = "yellow", label.size = NA, inherit.aes = FALSE) +
  labs(title = "AIRBNB Accommodations in Barcelona 2022",
