...
breaks <- classIntervals(world$Value, n = 5, style = "jenks")
breaks$brks[1] <- 0 

palette <- brewer.pal(n = 5, name = "YlOrRd")
europe_map <- ggplot(data = world) +
  geom_sf(aes(fill = cut(Value, breaks = breaks$brks)), color = "black") +
  scale_fill_manual(values = palette, na.value = "gray50", name = "Violence against Women") +
  labs(title = "Prevalence of Lifetime Violence against Women in Europe", subtitle = "Data Source: violence_women.csv") +
  theme_minimal()

print(europe_map)

ggsave("Europe_Violence_Against_Women_Map.png", plot = europe_map)
```
