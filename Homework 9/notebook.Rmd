...
map_list <- lapply(1:length(routes), function(i) {
  if (!is.null(routes[[i]])) {
    ...
    BCN_TILE +
      geom_sf(data = car_sf, aes(color = "Car"), size = 8, inherit.aes = FALSE) +
      geom_sf(data = foot_sf, aes(color = "Foot"), size = 8, inherit.aes = FALSE) +
      ...
      scale_color_manual(name = "Route Type", values = c("Car" = "red", "Foot" = "blue"))
  }
})
